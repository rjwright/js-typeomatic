...
--    putStrLn . show . getDeclarationGraph . label . toJSAST . parseTree $ pr
    putStrLn ""
    mapM_ ((\p c -> printASTChild c p False) (makeIndent ""))
        $
        (label . toJSAST . parseTree $ pr)
--    putStrLn ""
...
printCleanedRulesList [] _ = return()
...
printStringAndLabel str label False False = putStr str
...
printVarChild (var, label) padding printLabel printLine =
    printStringAndLabel (padding ++ " \"" ++ var ++ "\"") label printLabel printLine
...
maybePrintVarChild Nothing padding printLabel False =
    putStr (padding ++ " \"Nothing\"")
maybePrintVarChild Nothing padding printLabel True =
...
printIndexChild (index, label) padding printLabel printLine =
    printStringAndLabel
        (padding ++ " \"" ++ (show index) ++ "\"") label printLabel printLine


printValueChild :: ValueChild -> String -> PrintLabelFlag -> PrintLineFlag -> IO()
printValueChild (val, label) padding printLabel printLine =
    printLabelledValue val padding printLabel printLine


printOpChild :: OpChild -> String -> PrintLabelFlag -> PrintLineFlag -> IO()
printOpChild (op, label) padding printLabel printLine =
    printStringAndLabel
        (padding ++ " Operator \"" ++ op ++ "\"") label printLabel printLine
