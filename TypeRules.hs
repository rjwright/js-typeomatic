...
exprChildRules (LabIndex ex1 ex2, n) dIDs =
    ...
    -- has the same type as the statement.
        ...
        [Rule
            (Meta n)
            (ReferenceType (childToMeta ex1) (VariableProperty $ getStringLiteral ex2))
        ]
        ++
        [Rule
            (ReferenceType (childToMeta ex1) (VariableProperty $ getStringLiteral ex2))
...
exprChildRules (LabVarDeclaration var mex, n) dIDs =
    ...
    where
        ...
        maybeMetaRule x (Just expr) = [Rule (Meta x) (childToMeta expr)]
...
constructorRules block n =
    [Rule (ConstructorType (Meta n)) (ObjectType (concat $ map getAssignedProperty block))]
