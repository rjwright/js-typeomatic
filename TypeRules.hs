...
exprChildRules (LabList expList, n, sourceFragment) dIDs =
    [Rule (Meta n) (childWSToMeta $ last expList)]
...
exprChildRules (LabBinary ("+", _) ex1 ex2, n, sourceFragment) dIDs =
    [Rule (Meta n) (PlusType (childWSToMeta ex1) (childWSToMeta ex2))]
    ++ [Rule (PlusType (childWSToMeta ex1) (childWSToMeta ex2)) (Meta n)]
...
exprChildRules (LabBinary (op, _) ex1 ex2, n, sourceFragment) dIDs | elem op ["-", "%", "*"] =
    [Rule (childWSToMeta ex1) NumType]
    ++ [Rule (childWSToMeta ex2) NumType]
    ++ [Rule (Meta n) (IntAndInt (childWSToMeta ex1) (childWSToMeta ex2))]
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
...
exprChildRules (LabBinary (" in ", _) ex1 ex2, n, sourceFragment) dIDs =
    ...
    (if ((isIntLiteral ex2) || (isStringLiteral ex2)) then
        [Rule (childWSToMeta ex1) (AtLeastObjectType [])]

    else
        [Rule (childWSToMeta ex1) (AtLeastObjectType [])]
        -- If the object is not an array then type inference on the object and all of its members
        -- must fail.
        ++ [Rule (childWSToMeta ex1) (CorruptIfObjectType (childWSToMeta ex1))]
        ...
        -- an element.)
        ++ [Rule (childWSToMeta ex2) (IntIfArrayType (childWSToMeta ex1))])
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
...
exprChildRules (LabTernary ex1 ex2 ex3, n, sourceFragment) dIDs =
    [boolRule ex1]
    ++ [Rule (Meta n) (childWSToMeta ex2)]
    ++ [Rule (Meta n) (childWSToMeta ex3)]
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
...
exprChildRules (LabAssignment ("=", _) ex1 ex2, n, sourceFragment) dIDs =
    [Rule (Meta n) (childWSToMeta ex2)]
    ++ [Rule (childWSToMeta ex1) (childWSToMeta ex2)]
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
...
exprChildRules (LabAssignment (op, _) ex1 ex2, n, sourceFragment) dIDs | elem op ["/="] =
    [Rule (Meta n) FloatType]
    ++ [Rule (childWSToMeta ex1) FloatType]
    ++ [Rule (childWSToMeta ex2) NumType]
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
...
exprChildRules (LabReference ex1 ex2, n, sourceFragment) dIDs =
    [Rule (Meta n) (ReferenceType (childWSToMeta ex1) (getPropName ex2))]
    ++ [Rule (ReferenceType (childWSToMeta ex1) (getPropName ex2)) (Meta n)]
    -- The object must contain the property being referenced.
    ++ [Rule (childWSToMeta ex1) (AtLeastObjectType [(getPropName ex2)])]
    ++ (exprChildRules ex1 dIDs)
    where
...
exprChildRules (LabIndex ex1 ex2, n, sourceFragment) dIDs =
    ...
    (if (isIntLiteral ex2) then
        [Rule (Meta n) (ReferenceType (childWSToMeta ex1) (IndexProperty $ getIntLiteral ex2))]
        ++ [Rule (ReferenceType (childWSToMeta ex1) (IndexProperty $ getIntLiteral ex2)) (Meta n)]
        -- The object or array must contain the member being referenced.
        ++ [Rule (childWSToMeta ex1) (AtLeastObjectType [(IndexProperty $ getIntLiteral ex2)])]
    ...
    -- If the index is not an integer or sting literal then we cannot determine its value.
        ...
        [Rule (Meta n) (ReferenceType (childWSToMeta ex1) (UnknownProperty))]
        ++ [Rule (ReferenceType (childWSToMeta ex1) (UnknownProperty)) (Meta n)]
        -- If the object is not an array then type inference on the object and all of its members
        -- must fail.
        ++ [Rule (childWSToMeta ex1) (CorruptIfObjectType (childWSToMeta ex1))]
        -- If the object is an array then the index must have reference type (because we are, for
...
exprChildRules (LabCall ex1 ex2, n, sourceFragment) dIDs =
    [Rule (Meta n) (EvaluationType (childWSToMeta ex1))]
    -- The arguments passed to the function when calling it should match the parameters in the
    -- function definition.
    ++ [Rule (ArgumentsIDType (childWSToMeta ex1)) (ArgumentsType (argsToMeta ex2))]
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
    where
...
exprChildRules (LabCallExpression ex1 ("[]", _) ex2, n, sourceFragment) dIDs =
    ...
    -- thus type inference on the object fails.
        ...
        ++ [Rule (childWSToMeta ex1) AmbiguousType]
        ++ (exprChildRules ex2 dIDs))
...
exprChildRules (LabVarDeclaration var mex, n, sourceFragment) dIDs =
    ...
    where
        ...
        maybeMetaRule x (Just expr) = [Rule (Meta x) (childWSToMeta expr)]
...
exprChildRules (LabNew (LabCall ex1 ex2, p, sourceFragment1), n, sourceFragment2) dIDs =
    [Rule (Meta n) (InstantiationType (childWSToMeta ex1))]
...
blockRules block n =
    ...
    where
        ...
        getBlockRule (_, l, _) = [Rule (Meta n) (Meta l)]
...
funBodyRules block n =
    ...
    -- in the block.
        ...
        where
            ...
            getBlockRule (_, l, sourceFragment) = [Rule (ReturnType (Meta n)) (Meta l)]
...
bodyRule body n = Rule (Meta n) (childWSToMeta body)


-- Make a rule for expressions that have Boolean type
boolRule :: ExprChild -> Rule
boolRule (LabList ex, n, sourceFragment) = Rule (childWSToMeta $ last ex) BoolType
boolRule ex = Rule (childWSToMeta ex) BoolType
