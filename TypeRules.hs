...
-- unificaion algorithm in the context of type inference, although I have
-- written unification for the purpose of computing sufficientcies in a more
-- abstract context. I'm just hoping they will be in a convenient format for
-- unification.
...
-- unify with IntType to give FloatType (we can use ints as floats). But in
-- some contexts we require a variable to be strictly an integer (e.g. array
-- indexing). Possibly handle that in the compiler layer.


-- The types that can appear in the AST.
-- Some of them are second-order types which contain other types as fields.
-- The values of some second-order types depend of on the types of their
-- fields.
-- E.g:
...
data Type =
    ...
    | AmbiguousType
    -- ArgumentsIDType just contains a FunctionType (i.e. a reference back to
    -- the function it belongs to). I can't remember the exact details at this
    -- moment, but I'm pretty sure that rules containing this type tend to
    -- cause a cycle in the constraints which may cause problems.
    --
    -- TODO: Investigate
    ...
    | ArgumentsType [Type]
    -- An array has a list of properties (Elements! Not to be confused with
    -- other properties of the array object, which I am ignoring the existence
    -- of for now.)
    --
    -- TODO: Investigate and add more comments.
    | ArrayType [(PropertyName, Type)]
    -- When a member of an object is referenced it implies that the Object has
    -- at least the filed being referenced.
    | AtLeastObjectType [PropertyName]
    | BoolType
    -- ConstructorType contains the type variable for the body of a function.
    | ConstructorType Type
    -- For use when an object or array is indexed using a variable instead of
    -- a string literal or int literal. If the type of the field is found to
    -- be an object then that object has AmbiguousType. If the field is found
    -- to be an array then we do nothing with this information and proceed.
    ...
    -- Used for - * and % operators, where the value of the expression is an
    -- int if both operands ints and is a float otherwise.
    | IntAndInt Type Type
    ...
    -- equivalent to IntType. If the field turns out to be an object then we
    -- do nothing with this information and proceed.
    | IntIfArrayType Type
    | IntType
...
valueChildRules (LabObject members, x) dIDs =
    ...
    where
        ...
        prop (LabPropNameValue name ex, n) =
            ((getPropName $ name), (childToMeta $ ex))
        ...
        propTypeRules (LabPropNameValue name ex, n) =
            ...
            [Rule
                (ReferenceType thisObjectType (getPropName $ name))
                (Meta n)
            ]
...
-- Make a rule for the array. Make rules for the elements in the array. Make
-- rules for the values of the elements in the array.
--
-- TODO: Make sure that exprChildRules always binds n to something meaningfull
-- (n is the bridge from the element to the expression that is its value).
valueChildRules (LabArray elements, x) dIDs =
    [Rule (Meta x) (ArrayType (elemTypes elements [] 0))]
    ...
    where
        ...
        -- Pair the property name (i.e. element index) to the type of its
        -- value.
        elemType ex i = ((IndexProperty i), (childToMeta $ ex))
        -- Take a list of PropertyNameValues and make a list of indicies
        -- paired with the types of their values.
        elemTypes [] elTypes _ = elTypes
        elemTypes (e:els) elTypes i =
            elemTypes (els) ((elemType e i):elTypes) (i + 1)
        -- Each property (i.e. element) generates a rule relating the
        -- reference type to the type of its value.
        elemTypeRule ex i =
            Rule
                (ReferenceType
                    (ArrayType (elemTypes elements [] 0))
                    (IndexProperty i))
                (childToMeta $ ex)
        -- Find all rules generated by a list of array elements (properties).
        elemTypeRules [] elRules _ = elRules
        elemTypeRules (e:els) elRules i =
...
--
-- TODO: Go through and give the parameters for exprChildRules meaningfull
-- names!
...
exprChildRules (LabList expList, n) dIDs =
    [Rule (Meta n) (childToMeta . last $ expList)]
...
exprChildRules (LabBinary (op, _) ex1 ex2, n) dIDs | elem op ["/"] =
    ...
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
-- Bitwise binary operators act only on numbers (which are cast to integers).
...
exprChildRules (LabBinary (op, _) ex1 ex2, n) dIDs | elem op ["&", "|", "^"] =
    ...
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
-- Bitwise shift operators act only one numbers (which are cast to integers).
-- The type of the expression is integer.
--
-- FIXME: Pre-define these operator lists somewhere so that these signatures
-- aren't so long
...
exprChildRules (LabBinary (op, _) ex1 ex2, n) dIDs
    | elem op ["==", "!=", "===", "!==", ">", "<", ">=", "<="] =
        ...
        ++ (exprChildRules ex1 dIDs)
        ++ (exprChildRules ex2 dIDs)
-- The type of a binary logic expression is Bool. JavaScript's interpretation
-- of various expressions when cast to boolean is complex. What we want to do
-- with it depends on what we want to do with the compiler.
exprChildRules (LabBinary (op, _) ex1 ex2, n) dIDs | elem op ["&&", "||"] =
    [Rule (Meta n) BoolType]
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
-- Postfix '++' or '--' only operate on numbers. They type of the expression
-- is number (integer if ex is integer and float if ex is float).
exprChildRules (LabUnaryPost op ex, n) dIDs =
    [Rule (childToMeta $ ex) NumType]
    ++ [Rule (Meta n) (childToMeta $ ex)]
    ++ (exprChildRules ex dIDs)
-- These only operate on numbers. The type of the expression is the type of
-- ex.
exprChildRules (LabUnaryPre (op, _) ex, n) dIDs
    | elem op ["++", "--", "-", "+"] =
        [Rule (childToMeta $ ex) NumType]
        ++ [Rule (Meta n) (childToMeta $ ex)]
        ++ (exprChildRules ex dIDs)
...
exprChildRules (LabTernary ex1 ex2 ex3, n) dIDs =
    [boolRule $ ex1]
    ++ [Rule (Meta n) (childToMeta $ ex2)]
    ++ [Rule (Meta n) (childToMeta $ ex3)]
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
    ++ (exprChildRules ex3 dIDs)
-- They type of an assignment expression is the type of the value that is
-- being assigned to the variable. The type of the variable is the type if its
-- assigned value.
exprChildRules (LabAssignment ("=", _) ex1 ex2, n) dIDs =
    [Rule (Meta n) (childToMeta $ ex2)]
    ++ [Rule (childToMeta $ ex1) (childToMeta $ ex2)]
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
-- Similar to the '+' operator. This introduces a horrible cycle but I don't
-- think I can avoid it, because the new type of the variable depends on its
-- old type. The unification algorith will have to handle it.
exprChildRules (LabAssignment ("+=", _) ex1 ex2, n) dIDs =
    [Rule (Meta n) (PlusType (childToMeta $ ex1) (childToMeta $ ex2))]
...
-- but I don't think I can avoid it, as the new type of the variable depends
-- on its old type. The unification algorithm will have to handle it.
exprChildRules (LabAssignment (op, _) ex1 ex2, n) dIDs
    | elem op ["-=", "*=", "%="] =
        [Rule (Meta n) (IntAndInt (childToMeta $ ex1) (childToMeta $ ex2))]
        ++ [Rule (childToMeta $ ex1) (Meta n)]
        ++ [Rule (childToMeta $ ex2) NumType]
...
exprChildRules (LabAssignment (op, _) ex1 ex2, n) dIDs | elem op ["/="] =
    [Rule (Meta n) FloatType]
    ++ [Rule (childToMeta $ ex1) FloatType]
    ++ [Rule (childToMeta $ ex2) NumType]
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
-- Bitwise operator assignments. See bitwise operators above.
exprChildRules (LabAssignment op ex1 ex2, n) dIDs =
    [Rule (Meta n) IntType]
    ++ [Rule (childToMeta $ ex1) NumType]
    ++ [Rule (childToMeta $ ex2) NumType]
    ++ (exprChildRules ex1 dIDs)
...
exprChildRules (LabIdentifier var, n) dIDs =
    [Rule (Meta n) (childToMeta $ var)]
    ++ [Rule (childToMeta $ var) (Meta n)]
...
exprChildRules (LabReference ex1 ex2, n) dIDs =
    ...
    ++
    [Rule
        (ReferenceType (childToMeta $ ex1) (getPropName $ ex2))
        (Meta n)
    ]
...
exprChildRules (LabIndex ex1 ex2, n) dIDs =
    (exprChildRules ex1 dIDs)
    -- I don't really want to require the index to be an int literal or a
    -- string literal.In the case of arrays, we don't care about the types of
    -- particular elements, we justcare that all elements have the same type.
    -- Of course this is not the case for objects,since we need to know which
    -- property of the object we are modifying so we can check itstype (i.e.
    -- the *value* of the index matters).

    -- Having said that, accesses to previously non-existent elements of
    -- arrays after their creation simply "creates" the element (returns
    -- undefined on reads and adds the element the the array on writes, I'm
    -- pretty sure). If we define the type of an array to be array-type plus
    -- its size plus the type of its elements then adding elements to arrays
    -- after creation is not type safe. If we define the type of an array to
    -- be array-type plus the type of its elements then adding elements to an
    -- array after creation is type safe (provided sub- operations are type
    -- safe; e.g. the types of the elements remains homogeneous). Both are
    -- legitimate definitions. If we use the first definition then we require
    -- the user to define the array at its creation, which translates easily
    -- to C (as C doesn't automatically cope with dynamic arrays); but it
    -- means that we can't allow indexing of the array via anything but an
    -- integer literal. If we use the second definition then our C needs to
    -- implement dynamically expanding arrays; growing the array to accomodate
    -- any index that comes up during execution and filling any un- assigned
    -- elements to undefined. But it also means that we can support loops over
    -- arrays rather than failing type inference on the array whenever the
    -- user tries to loop over it. A big win! This is why this project needs a
    -- precise definition of type safety.

    -- Shane says that we should support dynamic arrays with elements that are
    -- all of the same type or undefined. For now I will just assume that all
    -- arrays have static size. I will also ignore the Array constructor for
    -- now and only deal with literally defined arrays.

    -- TODO: To make this work I will need to differentiate between references
    -- into arrays and references into objects.

    -- TODO: Also need to do something with the type of the index. Basically
    -- if the thing we're indexing into is an array then the index must be of
    -- type int. For the moment we will ignore (as in, pretend it can't
    -- happen) references to properties of array objects, other than elements
    -- of the array, using square brackets.
    ...
    (if (isIntLiteral $ ex2) then
        [Rule
            ...
            (ReferenceType
                (childToMeta $ ex1)
                (IndexProperty . getIntLiteral $ ex2)
            )
        ]
        ++
        [Rule
        ...
        [Rule
            (childToMeta $ ex1)
            (AtLeastObjectType [(IndexProperty . getIntLiteral $ ex2)])
        ]
    -- If the index is a string literal then the statement has type
    -- ReferenceType and the reference has the same type as the statement.
        ...
        [Rule
            ...
            (ReferenceType
                (childToMeta $ ex1)
                (VariableProperty . getStringLiteral $ ex2))
        ]
        ++
        [Rule
            (ReferenceType
...
exprChildRules (LabCall ex1 ex2, n) dIDs =
    [Rule (Meta n) (EvaluationType . childToMeta $ ex1)]
    -- The arguments passed to the function when calling it should match the
    -- parameters in the function definition.
    ++
    [Rule
        (ArgumentsIDType . childToMeta $ ex1)
        (ArgumentsType . argsToMeta $ ex2)
    ]
    ++ (exprChildRules ex1 dIDs)
    ++ (exprChildRules ex2 dIDs)
    where
...
-- Not much to say about arguments.
--
...
exprChildRules (LabParenExpression ex, n) dIDs =
    [Rule (Meta n) (childToMeta $ ex)]
    ++ [Rule (childToMeta $ ex) (Meta n)]
    ++ (exprChildRules ex dIDs)
-- TODO: Do I need to bind n to something? (is it ever a link in the chain?)
--
...
-- TODO: Do I need to bind n to something? (is it ever a link in the chain?)
--
-- Not much to say about continue statments. They don't really have a type and
-- I don't think that they can be used on the RHS of an assignment, nor can
-- they be returned or return anything (in the normal sense).
...
-- A CallExpression is a reference to a property of an evaluation of a
...
exprChildRules (LabCallExpression ex1 ("[]", _) ex2, n) dIDs =
    ...
    -- ReferenceType and the type of that reference is the type of they
    -- statement.
    (if (isIntLiteral $ ex2) then
        [Rule
            (Meta n)
    ...
    -- cannot know its value and thus type inference on the object fails.
        ...
        [Rule (Meta n) AmbiguousType]
        ++ [Rule (childToMeta ex1) AmbiguousType]
        ++ (exprChildRules ex2 dIDs)
    )
...
exprChildRules (LabFunctionExpression mv vls body, n) dIDs =
    ...
    where
        ...
        nameRule (Just (name, x)) (Just d) y =
            [Rule (Meta x) (makeIDType d)]
            ++ [Rule (makeIDType d) (Meta y)]
...
exprChildRules (LabVarDeclaration var mex, n) dIDs =
    ...
    (maybeMetaRule n mex)
    -- The type of the variable is equal to the type of mex if mex is not
    -- Nothing.
...
-- The type of a 'new' statment is they instantiation type of the function
-- (constructor) that it calls.
exprChildRules (LabNew (LabCall ex1 ex2, p), n) dIDs =
    [Rule (Meta n) (InstantiationType . childToMeta $ ex1)]
...
removeUselessParenAndList (LabParenExpression ex, _) =
...
--
-- Here I assume that calling removeUselessParenAndList is ok. But I should
-- allow the user to do {this.x = a, this.y = b;} which I think _might_ give
-- us a LabList containing two simple assignments. There are probably similar
-- problems with the use of removeUselessParenAndList elsewhere.
--
-- TODO: Check for and fix this
isSimpleAssignment :: ASTChild -> Bool
-- If the AST is a statement and is a simple assignment return true, false
-- otherwise.
isSimpleAssignment (LabStatement ex, _) =
...
assignmentGetVar (LabStatement ex, _) =
...
--
-- Can you wrap 'this' in parens or something? Can you assign a variable to
-- 'this' and then reference 'this' via the variable? e.g:
--  function Foo(b) {
--      var a = this;
--      if (b) { a.x = 5; }
--  }
...
-- function, both as a constructor and as a function.
--
...
-- FIXME: Change from this to the below. Is that OK?
-- getPropertyNameType (LabReference
--    (LabIdentifier (_, _), _)
--    (LabIdentifier (prop, _), _), n) =
getPropertyNameType (LabReference _ (LabIdentifier (prop, _), _), n) =
...
-- return [(VariableProperty "something", childToMeta ex1)].
...
-- information in the constructor body, the other (Haskell Land) functions
-- need to be able to deal with this.something. So when a JS program says
-- (this.something, n) = (2, m), all this function does is return (a
-- representation of) a property.
--
-- TODO: Other functions need to make rules that say things like
-- <type(this.something) = Meta n>, <Meta n = Meta m>, <Meta m = type(2)>.
-- (it's possible that this already works)
...
--
-- FIXME: This may be uneccessary. What if I only include properties declared
-- in simple assignments? Then if I reference any other property of an object
-- created with the constructor, type inference will fail *on that object*.
-- But not on the constructor. Does that matter?
--
-- Note that this problem is similar for the problem of return statments
-- inside child blocks.
--
...
-- safe.
--
-- We can only infer the return type of a function that (at least) has a
-- return statement in the top level of scope of its body. If all of its
-- return statements are inside inner ASTs then they might not get called.
-- Having said that, simple if-else routines, do-while loops, default routines
-- and finally routines always return if they contain a return statement. Do I
...
funBodyRules block n =
    ...
    -- relate the type of the block to UndefType and process all child ASTs in
    -- the block. If a child AST other than a function or an expression
    -- (wrapped in a Statment) contains a return statment with type other than
    -- undefined, then type inference on the function should fail.
    if (not (hasReturnType $ block)) then
        ([Rule (ReturnType (Meta n)) (UndefType)]
        ++ (concat . map getBlockRule $ block))
    ...
    -- process all child ASTs in the block.
        ...
        where
            ...
            getBlockRule (LabFunctionDeclaration _ _ _, _) = []
            -- Returns are ASTs and LabStatements can only contain ASTs in the
            -- form of function expression bodies, which can't return from
            -- 'this'. Return an empty list.
            getBlockRule (LabStatement _, _) = []
            -- For any other AST, the type of that AST (Meta l) equals the
            -- type of its body, although that rule is generated elsewhere.
            -- The type of 'this' equals the types of its child ASTs
            -- (excluding Statements and FunctionDeclarations.)
            getBlockRule (_, l) = [Rule (ReturnType (Meta n)) (Meta l)]
            -- Return true if the block contains a return statement at the top
            -- level of its scope.
...
--
-- Make a list of all properties declared in simple assignment statements
-- (after removing useless parens etc.). E.g. Say we find this.a = x,
-- this.b = y; We get [(this.a, n_a), (this.b, n_b)].
--
-- In future, this should check for references to properties (this.anything)
-- that aren't declared in the above. If such a reference is found, make a
-- rule [Rule (Meta n) (ConstructorType AmbiguosType)]. If no such reference
-- is found return a rule e.g.
-- [
-- Rule
--     (Meta n)
--     (ConstructorType
...
constructorRules block n =
    [Rule
        (ConstructorType (Meta n))
...
-- The type of the block is equal to the type of anything it returns
-- (blockRules).
astChildRules (LabBlock astList, n) dIDs =
    (mapASTChildRules astList dIDs) ++ (blockRules astList n)
-- The evaluation type of a function body is the same as the type of anything
-- it returns. It also has a construtor type - the type of the object that the
-- function makes when is instantiated.
astChildRules (LabFunctionBody astList, n) dIDs =
    (mapASTChildRules astList dIDs)
    ++ (funBodyRules astList n)
    ++ (constructorRules astList n)
-- Function declarations.
astChildRules (LabFunctionDeclaration var args body, n) dIDs =
    -- The type of the function's identifier is they same as they type of the
    ...
    ++ (varChildRules var dIDs)
    where
        -- The type of an argument in a function declaration is
        -- IdentifierType.
        makeIDType (DeclaredIdentifier ident lab) = IdentifierType ident lab
        ...
        thisFunID =
            IdentifierType
                (childGetValue $ var)
                (idGetLabel (childGetValue $ var) dIDs)
-- The type of a labelled block is the type of its body.
astChildRules (LabLabelled label body, n) dIDs =
    [bodyRule body n] ++ (varChildRules label dIDs)
    ++ (astChildRules body dIDs)
-- They type of a for loop is the type of its body. The test has boolean type.
astChildRules (LabForVar varEx test count body, n) dIDs =
    [bodyRule body n]
    ++ (maybeBoolRule $ test)
    ...
    ++ (astChildRules body dIDs)
-- They type of a for loop is the type of its body. The test has boolean type.
astChildRules (LabFor varEx test count body, n) dIDs =
    [bodyRule body n]
    ++ (maybeBoolRule $ test)
...
astChildRules (LabForIn varList obj body, n) dIDs =
    [bodyRule body n]
...
astChildRules (LabForVarIn varEx obj body, n) dIDs =
    [bodyRule body n]
...
-- boolean.
astChildRules (LabWhile test body, n) dIDs =
    [bodyRule body n]
    ++ [boolRule $ test]
    ++ (exprChildRules test dIDs)
    ++ (astChildRules body dIDs)
-- The type of a do-while loop is the type of its body. They type of the test
-- is bool.
astChildRules (LabDoWhile body test, n) dIDs =
    [bodyRule body n]
    ++ [boolRule $ test]
    ++ (exprChildRules test dIDs)
    ++ (astChildRules body dIDs)
...
-- is boolean.
astChildRules (LabIf test body, n) dIDs =
    [bodyRule body n]
    ++ [boolRule $ test]
    ++ (exprChildRules test dIDs)
    ++ (astChildRules body dIDs)
-- The type of an if-else construct is they type of both of its body blocks
-- (they must have the same type for type inference on the construct to
-- succeed). They type of the test is boolean.
astChildRules (LabIfElse test bodyT bodyF, n) dIDs =
    [bodyRule bodyT n]
    ++ [bodyRule bodyF n]
    ++ [boolRule $ test]
    ++ (exprChildRules test dIDs)
...
-- The type of a switch statment is the type of all of its cases (which in
-- turn have the type of their bodies). They type of all the cases must me the
-- same for type inference on the construct to succeed.
astChildRules (LabSwitch ident cases, n) dIDs =
    [bodyRule cases n]
    ++ (exprChildRules ident dIDs)
    ++ (astChildRules cases dIDs)
-- The type of a case in a switch statement is the same as the type of its
-- body.
--
-- TODO: ex has the same type as the variable in the outer switch
-- statement. Make rules to reflect this.
astChildRules (LabCase ex body, n) dIDs =
    [bodyRule body n]
    ++ (exprChildRules ex dIDs)
    ++ (astChildRules body dIDs)
-- The type of a default statment is the same as the type of its body.
astChildRules (LabDefault body, n) dIDs =
    [bodyRule body n]
    ++ (astChildRules body dIDs)
-- The type of a try construct is the same as the type of its body and the
-- type of its catch block. They must have the same type for type inference on
-- the construct to succeed.
astChildRules (LabTry body catches, n) dIDs =
    [bodyRule body n]
    ++ [bodyRule catches n]
    ++ (astChildRules body dIDs)
    ++ (astChildRules catches dIDs)
-- The type of a catch statment is the same as the type of its body. The type
-- of the test (usually used to conditionally catch an exception, e.g. only
-- catch an exception if it is equal to 3) is boolean.
astChildRules (LabCatch var mTest body, n) dIDs =
    [bodyRule body n]
    ++ (maybeBoolRule $ mTest)
    ++ (varChildRules var dIDs)
...
astChildRules (LabFinally body, n) dIDs =
    [bodyRule body n]
    ++ (astChildRules body dIDs)
-- They type of a return statement is they same as the type of the expression
-- it returns.
astChildRules (LabReturn ex, n) dIDs =
    [Rule (Meta n) (childToMeta $ ex)]
    ++ (exprChildRules ex dIDs)
-- They type of an instance of the Statment data type is the same as the type
-- of the expression it contains.
astChildRules (LabStatement ex, n) dIDs =
    [Rule (Meta n) (childToMeta $ ex)]
    ++ (exprChildRules ex dIDs)
