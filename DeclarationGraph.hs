...
-- unambiguous labels to all identifiers. Each node in the scope tree
-- represents a function or a function expression and includes the function's
-- identifier, the type constraints generated by that function, a list of
-- subtrees for any inner functions declared in that function, subtrees for
-- any function expressions declared in that function, and a list of
-- identifiers that are in scope inside that function's body. You can
-- uncomment some of the other functions in the main function to get some
-- different output. There's code for the scope tree with the AST subree that
-- generates each node included in that node, for a scope tree with the rules
-- field stripped, and for all the rules pulled out of the tree and lumped
-- into a list. Type rules are extracted and added to the tree as the tree is
-- built.
...
-- generated by the function, a list of functions delcared inside the body of
-- the function, a list of function expressions created in the body of the
-- function, a list of all identifiers visible to the function (for use in
-- generating rules with fully defined identifiers) and the chunk of AST where
-- the function is being declared (the parent).
--
-- Carrying the parent AST around seems like a waste of memory, but I don't
-- know how Haskell actually does it. I guess lazy eval means that it probably
-- doesn't actually exist all the time.
data FunctionRules =
    FunctionRules
        FunctionIdentifier
        [Rule]
...
-- Pretty much the same as FunctionRules, but for a function expression
...
class CleanedRules a where
    ...
    crCleanedFunctionExpressionRuleList ::
        a -> [CleanedFunctionExpressionRules]
...
instance CleanedRules CleanedFunctionRules where
    ...
    crCleanedFunctionExpressionRuleList
        (CleanedFunctionRules _ _ _ feRules _) = feRules
...
instance CleanedElement CleanedFunctionExpression where
    ...
    ceCleanedFunctionExpressionList
        (CleanedFunctionExpression _ _ feList _) = feList
    ceDeclaredIdentifierList (CleanedFunctionExpression _ _ _ dIDs) = dIDs
    ceName _ = "Function Expression"


-- Remove the parent field from a FunctionRules so that the tree is more
...
cleanFunctionRules (FunctionRules fid rules fRules feRules dIDs parent) =
    CleanedFunctionRules fid rules (map cleanFunctionRules fRules)
...
cleanFunctionExpressionRules
    (FunctionExpressionRules mid rules fRules feRules dIDs parent) =
        CleanedFunctionExpressionRules mid rules
...
cleanFunction (CleanedFunctionRules fid rules cfRules cfeRules dIDs) =
    CleanedFunction fid (map cleanFunction cfRules)
        (map cleanFunctionExpression cfeRules) dIDs


-- Remove the rules field from a CleanedFunctionExpressionRules so that the
-- tree is more legible when printed.
cleanFunctionExpression ::
    CleanedFunctionExpressionRules -> CleanedFunctionExpression
cleanFunctionExpression
    (CleanedFunctionExpressionRules mid rules cfRules cfeRules dIDs) =
        CleanedFunctionExpression mid (map cleanFunction cfRules)
            (map cleanFunctionExpression cfeRules) dIDs
...
funGetAllRules (FunctionRules fid rules funs funExs dIDs parent) old =
    ...
    where
        ...
        -- Map funExprGetAllRules over the function expressions declared in
        -- the parameter node and add them to funsRules
...
funExprGetAllRules
    (FunctionExpressionRules fid rules funs funExs dIDs parent) old =
        funExsRules
        where
            -- Add the rules in the parameter node to the old rules.
            currentRules = old ++ rules
...
mapFunExprGetAllRules (f:fx) rules =
    mapFunExprGetAllRules fx (funExprGetAllRules f rules)
...
-- expressed with unique identifiers so that they can later be extracted from
-- the tree and placed in a set without identifier collisions occuring.
...
getDeclarationGraph jsastLab =
    FunctionRules GlobalID (mapASTChildRules jsastLab dIDs)
...
-- Add two lists of identifiers. If an identifier occurs in both lists, keep
-- the element from the second list.
addDeclarationLists :: [DeclaredIdentifier] ->
...
addDeclarationLists old new =
    ...
    where
        ...
        -- Add the identifier from the old list if the new list doesn't
        -- contain it.
        addDec' o n = if listContains o n then [] else [o]
        -- Check if the new list contains an identifier.
        listContains _ [] = False
        listContains o (n:ns) =
            (identifiersMatch o n) || (listContains o ns)
        -- Check if two identifiers are the same (they will have unique
        -- labels, but we added those. We're looking for variables with the
        -- same name.)
        identifiersMatch
            (DeclaredIdentifier id1 x) (DeclaredIdentifier id2 y) =
                id1 == id2


-- Map astGetFunRules over a list of labelled AST nodes.
mapASTGetFR :: [ASTChild] ->
    ParentFunction -> [DeclaredIdentifier] -> [FunctionRules]
...
mapASTGetFER :: [ASTChild] ->
    ParentFunction -> [DeclaredIdentifier] -> [FunctionExpressionRules]
...
-- exception of LabFunctionDeclaration, LabReturn and LabStatement, just
-- process the body recursively.
astGetFunRules :: ASTChild ->
    ParentFunction -> [DeclaredIdentifier] -> [FunctionRules]
astGetFunRules (LabBlock astList, n) parent dIDs =
    (mapASTGetFR astList parent dIDs)
astGetFunRules (LabFunctionBody astList, n) parent dIDs =
    (mapASTGetFR astList parent dIDs)
...
astGetFunRules (LabFunctionDeclaration (fid, x) args body, n) parent dIDs =
    ...
    where
        ...
        -- quite inelegant, but I don't think there's really any way around
        -- it. I think haskell should automatically bind a varible to function
        -- parameters.
        ...
        -- calls to astChildRules, astGetFunRule and astGetFunExprRules for
        -- this function's body.
        declaredIDs =
            (addDeclarationLists dIDs (funDecGetVarDecs thisFunction))
        -- The parent AST for this function declaration.
        newParent = ParentFunDecl thisFunction
astGetFunRules (LabLabelled label body, n) parent dIDs =
    astGetFunRules body parent dIDs
astGetFunRules (LabForVar varEx test count body, n) parent dIDs =
    astGetFunRules body parent dIDs
astGetFunRules (LabFor varEx test count body, n) parent dIDs =
    astGetFunRules body parent dIDs
astGetFunRules (LabForIn varList obj body, n) parent dIDs =
    astGetFunRules body parent dIDs
astGetFunRules (LabForVarIn varEx obj body, n) parent dIDs =
    astGetFunRules body parent dIDs
astGetFunRules (LabWhile test body, n) parent dIDs =
    astGetFunRules body parent dIDs
astGetFunRules (LabDoWhile body test, n) parent dIDs =
    astGetFunRules body parent dIDs
astGetFunRules (LabIf test body, n) parent dIDs =
    astGetFunRules body parent dIDs
astGetFunRules (LabIfElse test bodyT bodyF, n) parent dIDs =
    (astGetFunRules bodyT parent dIDs)
    ++ (astGetFunRules bodyF parent dIDs)
astGetFunRules (LabSwitch ident cases, n) parent dIDs =
    astGetFunRules cases parent dIDs
astGetFunRules (LabCase ex body, n) parent dIDs =
    astGetFunRules body parent dIDs
astGetFunRules (LabDefault body, n) parent dIDs =
    astGetFunRules body parent dIDs
astGetFunRules (LabTry body catches, n) parent dIDs =
    (astGetFunRules body parent dIDs)
    ++ (astGetFunRules catches parent dIDs)
astGetFunRules (LabCatch var mTest body, n) parent dIDs =
    astGetFunRules body parent dIDs
astGetFunRules (LabFinally body, n) parent dIDs =
    astGetFunRules body parent dIDs
-- LabReturn has only one field, an expression. And expression can't
-- (deirectly) contain a function declaration. Return nothing.
astGetFunRules (LabReturn ex, n) parent dIDs = []
-- LabStatement has only one field, an expression. And expression can't
-- (deirectly) contain a function declaration. Return nothing.
...
-- expressions.
astGetFunExprRules :: ASTChild ->
    ParentFunction -> [DeclaredIdentifier] -> [FunctionExpressionRules]
astGetFunExprRules (LabBlock astList, n) parent dIDs =
    (mapASTGetFER astList parent dIDs)
astGetFunExprRules (LabFunctionBody astList, n) parent dIDs =
    (mapASTGetFER astList parent dIDs)
astGetFunExprRules (LabFunctionDeclaration fid args body, n) parent dIDs = []
astGetFunExprRules (LabLabelled label body, n) parent dIDs =
    astGetFunExprRules body parent dIDs
astGetFunExprRules (LabForVar varEx test count body, n) parent dIDs =
    (astGetFunExprRules body parent dIDs)
    ++ (mapExpGetFER varEx parent dIDs)
    ++ (getMaybeFunExprRules test parent dIDs)
    ++ (getMaybeFunExprRules count parent dIDs)
astGetFunExprRules (LabFor varEx test count body, n) parent dIDs =
    (astGetFunExprRules body parent dIDs)
    ++ (getMaybeFunExprRules varEx parent dIDs)
    ++ (getMaybeFunExprRules test parent dIDs)
    ++ (getMaybeFunExprRules count parent dIDs)
astGetFunExprRules (LabForIn varList obj body, n) parent dIDs =
    (astGetFunExprRules body parent dIDs)
    ++ (exprGetFunExprRules obj parent dIDs)
astGetFunExprRules (LabForVarIn varEx obj body, n) parent dIDs =
    (astGetFunExprRules body parent dIDs)
    ++ (exprGetFunExprRules obj parent dIDs)
    ++ (exprGetFunExprRules varEx parent dIDs)
astGetFunExprRules (LabWhile test body, n) parent dIDs =
    (astGetFunExprRules body parent dIDs)
    ++ (exprGetFunExprRules test parent dIDs)
astGetFunExprRules (LabDoWhile body test, n) parent dIDs =
    (astGetFunExprRules body parent dIDs)
    ++ (exprGetFunExprRules test parent dIDs)
astGetFunExprRules (LabIf test body, n) parent dIDs =
    (astGetFunExprRules body parent dIDs)
    ++ (exprGetFunExprRules test parent dIDs)
astGetFunExprRules (LabIfElse test bodyT bodyF, n) parent dIDs =
    (astGetFunExprRules bodyT parent dIDs)
    ++ (astGetFunExprRules bodyF parent dIDs)
    ++ (exprGetFunExprRules test parent dIDs)
astGetFunExprRules (LabSwitch ident cases, n) parent dIDs =
    (astGetFunExprRules cases parent dIDs)
    ++ (exprGetFunExprRules ident parent dIDs)
astGetFunExprRules (LabCase ex body, n) parent dIDs =
    (astGetFunExprRules body parent dIDs)
    ++ (exprGetFunExprRules ex parent dIDs)
astGetFunExprRules (LabDefault body, n) parent dIDs =
    astGetFunExprRules body parent dIDs
astGetFunExprRules (LabTry body catches, n) parent dIDs =
    (astGetFunExprRules body parent dIDs)
    ++ (astGetFunExprRules catches parent dIDs)
astGetFunExprRules (LabCatch var mTest body, n) parent dIDs =
    (astGetFunExprRules body parent dIDs)
    ++ (getMaybeFunExprRules mTest parent dIDs)
astGetFunExprRules (LabFinally body, n) parent dIDs =
    astGetFunExprRules body parent dIDs
astGetFunExprRules (LabReturn ex, n) parent dIDs =
    exprGetFunExprRules ex parent dIDs
astGetFunExprRules (LabStatement ex, n) parent dIDs =
    exprGetFunExprRules ex parent dIDs


-- Does this make sense with the rules code that handles members of objects
-- and arrays?
--
-- TODO: Search object and array literals for function expressions.
valueGetFunExprRules :: ValueChild ->
    ParentFunction -> [DeclaredIdentifier] -> [FunctionExpressionRules]
valueGetFunExprRules (LabObject ex, n) parent dIDs =
    mapExpGetFER ex parent dIDs
valueGetFunExprRules (LabArray els, n) parent dIDs =
    mapExpGetFER els parent dIDs
...
-- contain any fields that could contain a function expression), or
-- recursively process any expression or value fields.
exprGetFunExprRules :: ExprChild ->
    ParentFunction -> [DeclaredIdentifier] -> [FunctionExpressionRules]
exprGetFunExprRules (LabList expList, n) parent dIDs =
    mapExpGetFER expList parent dIDs
exprGetFunExprRules (LabBinary op ex1 ex2, n) parent dIDs =
    (exprGetFunExprRules ex1 parent dIDs)
    ++ (exprGetFunExprRules ex2 parent dIDs)
exprGetFunExprRules (LabUnaryPost op ex, n) parent dIDs =
    exprGetFunExprRules ex parent dIDs
exprGetFunExprRules (LabUnaryPre op ex, n) parent dIDs =
    exprGetFunExprRules ex parent dIDs
exprGetFunExprRules (LabTernary ex1 ex2 ex3, n) parent dIDs =
    (exprGetFunExprRules ex1 parent dIDs)
    ++ (exprGetFunExprRules ex2 parent dIDs)
    ++ (exprGetFunExprRules ex3 parent dIDs)
exprGetFunExprRules (LabAssignment op ex1 ex2, n) parent dIDs =
    (exprGetFunExprRules ex1 parent dIDs)
    ++ (exprGetFunExprRules ex2 parent dIDs)
exprGetFunExprRules (LabIdentifier var, n) parent dIDs = []
exprGetFunExprRules (LabReference ex1 ex2, n) parent dIDs =
    (exprGetFunExprRules ex1 parent dIDs)
    ++ (exprGetFunExprRules ex2 parent dIDs)
exprGetFunExprRules (LabIndex ex1 ex2, n) parent dIDs =
    (exprGetFunExprRules ex1 parent dIDs)
    ++ (exprGetFunExprRules ex2 parent dIDs)
-- TODO: Needs to deal with objects and arrays, whose members can be function
-- expressions.
--
...
exprGetFunExprRules (LabValue val, n) parent dIDs =
    valueGetFunExprRules val parent dIDs
exprGetFunExprRules (LabPropNameValue var ex, n) parent dIDs =
    exprGetFunExprRules ex parent dIDs
exprGetFunExprRules (LabCall ex1 ex2, n) parent dIDs =
    (exprGetFunExprRules ex1 parent dIDs)
    ++ (exprGetFunExprRules ex2 parent dIDs)
exprGetFunExprRules (LabArguments args, n) parent dIDs =
    mapExpGetFER args parent dIDs
exprGetFunExprRules (LabParenExpression ex, n) parent dIDs =
    exprGetFunExprRules ex parent dIDs
exprGetFunExprRules (LabBreak var, n) parent dIDs = []
exprGetFunExprRules (LabContinue var, n) parent dIDs = []
exprGetFunExprRules (LabThrow ex, n) parent dIDs =
    exprGetFunExprRules ex parent dIDs
exprGetFunExprRules (LabCallExpression ex1 op ex2, n) parent dIDs =
    (exprGetFunExprRules ex1 parent dIDs)
    ++ (exprGetFunExprRules ex2 parent dIDs)
...
exprGetFunExprRules (LabFunctionExpression mv vls body, n) parent dIDs =
    ...
    [FunctionExpressionRules
        ...
        (astChildRules body declaredIDs)
        -- Find all function declarations in this function expression's body
        -- and process recursively.
        (astGetFunRules body newParent declaredIDs)
        -- Find all function expressions in this function expression's body
        -- and process recursively.
        (astGetFunExprRules body newParent declaredIDs)
        -- Declared identifiers visible inside this function expression's
        -- body, plus this function expression's parent AST.
        declaredIDs parent
    ]
    where
        -- Give a name to the (Haskell Land) argument.
        ...
        -- neccessary. Resulting list of declared identifiers is passed into
        -- calls to astChildRules, astGetFunRule and astGetFunExprRules for
        -- this function expression's body.
...
exprGetFunExprRules (LabVarDeclaration var mex, n) parent dIDs =
    getMaybeFunExprRules mex parent dIDs
exprGetFunExprRules (LabNew ex, n) parent dIDs =
    exprGetFunExprRules ex parent dIDs


-- Make FunctionExpressionRules from Maybe ExprChild.
getMaybeFunExprRules :: (Maybe ExprChild) ->
    ParentFunction -> [DeclaredIdentifier] -> [FunctionExpressionRules]
getMaybeFunExprRules Nothing parent dIDs = []
getMaybeFunExprRules (Just ex) parent dIDs =
    exprGetFunExprRules ex parent dIDs
...
--
-- NOTE: Argument lists declare variables (see testargscope.js). DONE - see
-- funDecGetVarDecs.
--
...
exprGetVarDecs (LabCallExpression ex1 op ex2, n) =
    (exprGetVarDecs $ ex1)
    ++ (exprGetVarDecs $ ex2)
-- A function expression doesn't declare anything that is relevant at this
-- scope.
exprGetVarDecs (LabFunctionExpression mv var body, n)  = []
-- Return the unique identifier for the variable being declared and
-- recursively process the expression field.
exprGetVarDecs (LabVarDeclaration (var, x) mex, n) =
...
-- Find all identifiers declared in a JSAST. All of these, with the exception
-- of LabFunctionDeclaration and LabLabelled, return nothing or recursively
-- process any AST or expression fields.
...
astGetVarDecs (LabFunctionDeclaration (var, x) args body, n) =
    [funDecMakeLabel thisFun]
    where
        thisFun = (LabFunctionDeclaration (var, x) args body, n)
-- FIXME: Not really sure about this.
astGetVarDecs (LabLabelled (var, x) body, n) =
    [labelledMakeLabel thisLabelled]
    ++ (astGetVarDecs $ body)
    where
        thisLabelled = (LabLabelled (var, x) body, n)
astGetVarDecs (LabForVar ex mex1 mex2 body, n) =
    (concat . map exprGetVarDecs $ ex)
    ++ (maybeExprGetVarDecs $ mex1)
    ++ (maybeExprGetVarDecs $ mex2)
    ++ (astGetVarDecs $ body)
astGetVarDecs (LabFor mex1 mex2 mex3 body, n) =
    (maybeExprGetVarDecs $ mex1)
    ++ (maybeExprGetVarDecs $ mex2)
    ++ (maybeExprGetVarDecs $ mex3)
    ++ (astGetVarDecs $ body)
astGetVarDecs (LabForIn vars ex body, n) =
    (exprGetVarDecs $ ex)
    ++ (astGetVarDecs $ body)
astGetVarDecs (LabForVarIn ex1 ex2 body, n) =
    (exprGetVarDecs $ ex1)
    ++ (exprGetVarDecs $ ex2)
    ++ (astGetVarDecs $ body)
astGetVarDecs (LabWhile ex body, n) =
    (exprGetVarDecs $ ex)
    ++ (astGetVarDecs $ body)
astGetVarDecs (LabDoWhile body ex, n) =
    (astGetVarDecs $ body)
    ++ (exprGetVarDecs $ ex)
astGetVarDecs (LabIf ex body, n) =
    (exprGetVarDecs $ ex)
    ++ (astGetVarDecs $ body)
astGetVarDecs (LabIfElse ex bodyT bodyF, n) =
    (exprGetVarDecs $ ex)
    ++ (astGetVarDecs $ bodyT)
    ++ (astGetVarDecs $ bodyF)
astGetVarDecs (LabSwitch ex cases, n) =
    (exprGetVarDecs $ ex)
    ++ (astGetVarDecs $ cases)
astGetVarDecs (LabCase ex body, n) =
    (exprGetVarDecs $ ex)
    ++ (astGetVarDecs $ body)
astGetVarDecs (LabDefault body, n) = astGetVarDecs $ body
astGetVarDecs (LabTry body catch, n) =
    (astGetVarDecs $ body)
    ++ (astGetVarDecs $ catch)
astGetVarDecs (LabCatch var mex body, n) =
    (maybeExprGetVarDecs $ mex)
    ++ (astGetVarDecs $ body)
